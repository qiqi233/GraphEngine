...
#include "Mesh/Core/GeometryGenerator.h"
...
FRenderingPipeline::FRenderingPipeline(FDX12RenderEngine* RenderEngine)
{
	DX12RenderEngine=RenderEngine;
...
void FRenderingPipeline::BuildRootSignature()
{
	...
	DX12_DEBUG_MESSAGE(GetD3dDevice()->CreateRootSignature(
		0,
...
void FRenderingPipeline::BuildShadersAndInputLayout()
{
	//构建输入布局

	mShaders["VS"] = FShader::BuildShaders(L"../GraphEngine/Shader/testShader.hlsl", "VSMain", "vs_5_1");
	mShaders["PS"] = FShader::BuildShaders(L"../GraphEngine/Shader/testShader.hlsl", "PSMain", "ps_5_1");
	...
	mInputLayout =
	{
		...
		{"TEXTURE", 0, DXGI_FORMAT_R32G32_FLOAT, 0, 52, D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA, 0}
	};
	//mInputLayout =
	//{
	//	{ "POSITION", 0, DXGI_FORMAT_R32G32B32_FLOAT, 0, 0, D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA, 0 },
...
void FRenderingPipeline::BuildGeometry()
{
	GeometryGenerator geoGen;
	FMeshData box = geoGen.CreateBox(1.5f, 0.5f, 1.5f, 3);
	FMeshData grid = geoGen.CreateGrid(20.0f, 30.0f, 60, 40);
	FMeshData sphere = geoGen.CreateSphere(0.5f, 20, 20);
	FMeshData cylinder = geoGen.CreateCylinder(0.5f, 0.3f, 3.0f, 20, 20);

	//
	//我们将所有几何体连接到一个大的顶点/索引缓冲区中。所以
	//定义每个子网格覆盖的缓冲区中的区域。
	//
	//缓存连接顶点缓冲区中每个对象的顶点偏移。
	...
	std::vector<FVertexData> vertices(totalVertexCount);
	...
	const UINT vbByteSize = (UINT)vertices.size() * sizeof(FVertexData);
	const UINT ibByteSize = (UINT)indices.size() * sizeof(std::uint16_t);

	auto geo = std::make_unique<IRenderingIntface>();
	geo->Name = "shapeGeo";

	DX12_DEBUG_MESSAGE(D3DCreateBlob(vbByteSize, &geo->VertexBufferCPU));
	CopyMemory(geo->VertexBufferCPU->GetBufferPointer(), vertices.data(), vbByteSize);

	DX12_DEBUG_MESSAGE(D3DCreateBlob(ibByteSize, &geo->IndexBufferCPU));
	CopyMemory(geo->IndexBufferCPU->GetBufferPointer(), indices.data(), ibByteSize);

	geo->VertexBufferGPU = d3dUtil::CreateDefaultBuffer(GetD3dDevice().Get(),
		GetGraphicsCommandList().Get(), vertices.data(), vbByteSize, geo->VertexBufferUploader);

	geo->IndexBufferGPU = d3dUtil::CreateDefaultBuffer(GetD3dDevice().Get(),
		GetGraphicsCommandList().Get(), indices.data(), ibByteSize, geo->IndexBufferUploader);

	geo->VertexByteStride = sizeof(FVertexData);
...
void FRenderingPipeline::BuildRenderItems()
{
...
void FRenderingPipeline::BuildFrameResources()
{
...
void FRenderingPipeline::BuildDescriptorHeaps()
{
	...
	DX12_DEBUG_MESSAGE( GetD3dDevice()->CreateDescriptorHeap(&cbvHeapDesc,
		IID_PPV_ARGS(&mCbvHeap)));
}

void FRenderingPipeline::BuildConstantBufferViews()
{
...
void FRenderingPipeline::BuildPSOs()
{
	...
	opaquePsoDesc.RTVFormats[0] = DX12RenderEngine->BackBufferFormat;
	opaquePsoDesc.SampleDesc.Count = DX12RenderEngine->GetDXGISampleCount();
	opaquePsoDesc.SampleDesc.Quality = DX12RenderEngine->GetDXGISampleQuality();
	opaquePsoDesc.DSVFormat = DX12RenderEngine->DepthStencilFormat;
	HRESULT hr=GetD3dDevice()->CreateGraphicsPipelineState(&opaquePsoDesc, IID_PPV_ARGS(&mPSOs["opaque"]));
	DX12_DEBUG_MESSAGE(hr );
	...
	DX12_DEBUG_MESSAGE( GetD3dDevice()->CreateGraphicsPipelineState(&opaqueWireframePsoDesc, IID_PPV_ARGS(&mPSOs["opaque_wireframe"])));
}

void FRenderingPipeline::OnKeyboardInput(const GameTimer& gt)
{
...
void FRenderingPipeline::Update(const GameTimer& gt)
{
	...
	if (mCurrFrameResource->Fence != 0 && DX12RenderEngine->D3dFence->GetCompletedValue() < mCurrFrameResource->Fence)
	{
		HANDLE eventHandle = CreateEventEx(NULL, NULL, 0, EVENT_ALL_ACCESS);
		DX12_DEBUG_MESSAGE(DX12RenderEngine->D3dFence->SetEventOnCompletion(mCurrFrameResource->Fence, eventHandle));
...
void FRenderingPipeline::DrawRenderItems(ID3D12GraphicsCommandList* cmdList)
{
	//设置常量描述符堆
