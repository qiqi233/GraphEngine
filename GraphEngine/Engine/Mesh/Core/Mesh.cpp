...
// UMesh* UMesh::CreateMesh(const FMeshRenderingData* InData)
// {
// 	UMesh* NewMesh=new UMesh;
// 	NewMesh->BuildMesh(InData);
// 	return NewMesh;
// }
...
void UMesh::BuildMesh(const FMeshRenderingData* InData)
{
	...
	//设置采样级别
	psoDesc.SampleDesc.Count = GetEngine()->GetDXGISampleCount();
	psoDesc.SampleDesc.Quality = GetEngine()->GetDXGISampleQuality();

	//设置RTV格式
	psoDesc.RTVFormats[0] = GetEngine()->GetBackBufferFormat();
	//设置深度/模板格式
	psoDesc.DSVFormat = GetEngine()->GetDepthStencilFormat();
...
D3D12_VERTEX_BUFFER_VIEW UMesh::GetVertexBufferView()
{
...
D3D12_INDEX_BUFFER_VIEW UMesh::GetIndexBufferView()
{
