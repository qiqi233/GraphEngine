...
namespace UEMath
{
	...
	*/
	...
	public:
		 ...
		 */
		explicit inline FVector2D(EForceInit);
		 ...
		 */
		explicit inline FVector2D(ENoInit) { }
		 ...
		 * @param V Vector to copy from.
		 */
		explicit inline FVector2D(const FVector4& V);
	 ...
	 */
	//inline uint32 GetTypeHash(const FVector2D& Vector)
	//{
	...
	inline FVector2D::FVector2D(EForceInit)
		: X(0), Y(0)
	{
	}
	...
	inline float& FVector2D::Component(int32 Index)
	{
		return (&X)[Index];
	}


	inline float FVector2D::Component(int32 Index) const
	{
		return (&X)[Index];
	}
	...
	inline float FMath::GetRangePct(FVector2D const& Range, float Value)
	{
		return GetRangePct(Range.X, Range.Y, Value);
	}

	inline float FMath::GetRangeValue(FVector2D const& Range, float Pct)
	{
