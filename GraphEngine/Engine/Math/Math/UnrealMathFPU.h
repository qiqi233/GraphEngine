...
#define VectorNegate( Vec )				MakeVectorRegister( -(Vec).V[0], -(Vec).V[1], -(Vec).V[2], -(Vec).V[3] )
	...
	*/
	 ...
	 */
	inline int VectorCompareZero(const VectorRegister& Mask)
	{
		return
			(Mask.V[0] >= 0 ? 0x1 : 0) +
			(Mask.V[1] >= 0 ? 0x2 : 0) +
...
#define VectorIntSign(A) VectorIntSelect( VectorIntCompareGE(A, GlobalVectorConstants::IntZero), GlobalVectorConstants::IntOne, GlobalVectorConstants::IntMinusOne )
	...
	*/
	 ...
	 */
	inline VectorRegister VectorMask_LT(const VectorRegister& Vec1, const VectorRegister& Vec2)
	{
		return VectorCompareLT(Vec1, Vec2);
	}

	inline VectorRegister VectorMask_LE(const VectorRegister& Vec1, const VectorRegister& Vec2)
	{
		return VectorCompareLE(Vec1, Vec2);
	}

	inline VectorRegister VectorMask_GT(const VectorRegister& Vec1, const VectorRegister& Vec2)
	{
		return VectorCompareGT(Vec1, Vec2);
	}

	inline VectorRegister VectorMask_GE(const VectorRegister& Vec1, const VectorRegister& Vec2)
	{
		return VectorCompareGE(Vec1, Vec2);
	}

	inline VectorRegister VectorMask_EQ(const VectorRegister& Vec1, const VectorRegister& Vec2)
	{
		return VectorCompareEQ(Vec1, Vec2);
	}

	inline VectorRegister VectorMask_NE(const VectorRegister& Vec1, const VectorRegister& Vec2)
	{
	 ...
	 * @return				Bit 0 = sign(VecMask.x), Bit 1 = sign(VecMask.y), Bit 2 = sign(VecMask.z), Bit 3 = sign(VecMask.w)
	 */
	inline int32_t VectorMaskBits(const VectorRegister& Vec1)
	{
		uint32* V1 = (uint32*)(&(Vec1.V[0]));
