 ...
 */
	struct FMath : public FPlatformMath
	{
		...
		static inline int32_t RandHelper(int32_t A)
		{
		...
		static inline int64_t RandHelper64(int64_t A)
		{
			// Note that on some platforms RAND_MAX is a large number so we cannot do ((rand()/(RAND_MAX+1)) * A)
			// or else we may include the upper bound results, which should be excluded.
		...
		static inline int32_t RandRange(int32_t Min, int32_t Max)
		{
			const int32_t Range = (Max - Min) + 1;
			return Min + RandHelper(Range);
		}

		static inline int64_t RandRange(int64_t Min, int64_t Max)
		{
		...
		/** Snaps a value to the nearest grid multiple */
		static inline double GridSnap(double Location, double Grid)
		{
...
#undef FASTASIN_HALF_PI
		...
		static inline void CartesianToPolar(const float X, const float Y, float& OutRad, float& OutAng)
		{
			OutRad = Sqrt(Square(X) + Square(Y));
			OutAng = Atan2(Y, X);
		}
		/** Converts given Cartesian coordinate pair to Polar coordinate system. */
		static inline void CartesianToPolar(const FVector2D InCart, FVector2D& OutPolar);

		/** Converts given Polar coordinate pair to Cartesian coordinate system. */
		static inline void PolarToCartesian(const float Rad, const float Ang, float& OutX, float& OutY)
		{
			OutX = Rad * Cos(Ang);
			OutY = Rad * Sin(Ang);
		}
		/** Converts given Polar coordinate pair to Cartesian coordinate system. */
		static inline void PolarToCartesian(const FVector2D InPolar, FVector2D& OutCart);
		...
		static inline float GetRangePct(float MinValue, float MaxValue, float Value)
		{
		...
		static inline float GetMappedRangeValueClamped(const FVector2D& InputRange, const FVector2D& OutputRange, const float Value)
		{
		...
		static inline float GetMappedRangeValueUnclamped(const FVector2D& InputRange, const FVector2D& OutputRange, const float Value)
		{
		...
		template< class T >
		static inline T LerpStable(const T& A, const T& B, double Alpha)
		{
		...
		/** Performs a linear interpolation between two values, Alpha ranges from 0-1. Handles full numeric range of T */
		template< class T >
		static inline T LerpStable(const T& A, const T& B, float Alpha)
		{
		 ...
		 */
		template< class T, class U >
		static inline T CubicInterpDerivative(const T& P0, const T& T0, const T& P1, const T& T1, const U& A)
		{
		 ...
		 */
		template< class T, class U >
		static inline T CubicInterpSecondDerivative(const T& P0, const T& T0, const T& P1, const T& T1, const U& A)
		{
		...
		template< class T >
		static inline T InterpEaseIn(const T& A, const T& B, float Alpha, float Exp)
		{
		...
		template< class T >
		static inline T InterpEaseOut(const T& A, const T& B, float Alpha, float Exp)
		{
		...
		template< class T >
		static inline T InterpEaseInOut(const T& A, const T& B, float Alpha, float Exp)
		{
		...
		template< class T >
		static inline T InterpStep(const T& A, const T& B, float Alpha, int32_t Steps)
		{
			if (Steps <= 1 || Alpha <= 0)
			{
				return A;
		...
		template< class T >
		static inline T InterpSinIn(const T& A, const T& B, float Alpha)
		{
		...
		template< class T >
		static inline T InterpSinOut(const T& A, const T& B, float Alpha)
		{
		...
		template< class T >
		static inline T InterpSinInOut(const T& A, const T& B, float Alpha)
		{
			return Lerp<T>(A, B, (Alpha < 0.5f) ?
		...
		template< class T >
		static inline T InterpExpoIn(const T& A, const T& B, float Alpha)
		{
		...
		template< class T >
		static inline T InterpExpoOut(const T& A, const T& B, float Alpha)
		{
		...
		template< class T >
		static inline T InterpExpoInOut(const T& A, const T& B, float Alpha)
		{
			return Lerp<T>(A, B, (Alpha < 0.5f) ?
		...
		template< class T >
		static inline T InterpCircularIn(const T& A, const T& B, float Alpha)
		{
		...
		template< class T >
		static inline T InterpCircularOut(const T& A, const T& B, float Alpha)
		{
			Alpha -= 1.f;
		...
		template< class T >
		static inline T InterpCircularInOut(const T& A, const T& B, float Alpha)
		{
			return Lerp<T>(A, B, (Alpha < 0.5f) ?
		 ...
		 */
		template< class U >
		static inline U CubicCRSplineInterp(const U& P0, const U& P1, const U& P2, const U& P3, const float T0, const float T1, const float T2, const float T3, const float T)
		{
		...
		template< class U >
		static inline U CubicCRSplineInterpSafe(const U& P0, const U& P1, const U& P2, const U& P3, const float T0, const float T1, const float T2, const float T3, const float T)
		{
			//Based on http://www.cemyuksel.com/research/catmullrom_param/catmullrom.pdf 
		...
		static uint32_t ComputeProjectedSphereScissorRect(struct FIntRect& InOutScissorRect, FVector SphereOrigin, float Radius, FVector ViewOrigin, const FMatrix& ViewMatrix, const FMatrix& ProjMatrix);
		 ...
		 */
		static int32_t PlaneAABBRelativePosition(const FPlane& P, const FBox& AABB);
		...
		*/
		 ...
		 */
		static bool PointsAreCoplanar(const std::vector<FVector>& Points, const float Tolerance = 0.1f);
		...
		*/
		 ...
		 */
		static std::string FormatIntToHumanReadable(int32_t Val);
		 ...
		 */
		static bool MemoryTest(void* BaseAddress, uint32_t NumBytes);
		 ...
		 */
		static bool Eval(std::string Str, float& OutValue);
		...
		static const uint32_t BitFlag[32];
		...
		*/
		 ...
		 */
		static inline float WeightedMovingAverage(float CurrentSample, float PreviousSample, float Weight)
		{
		 ...
		 * @return the next value in the series
		 */
		static inline float DynamicWeightedMovingAverage(float CurrentSample, float PreviousSample, float MaxDistance, float MinWeight, float MaxWeight)
		{
