...
namespace UEMath
{
	...
	public:
		 ...
		 */
		explicit inline FQuat(EForceInit);
	...
	inline FQuat::FQuat(EForceInit ZeroOrNot)
	...
	inline std::string FQuat::ToString() const
	{
		char stemp[200] = "";
		snprintf(stemp, 200, "X=%.9f Y=%.9f Z=%.9f W=%.9f", X, Y, Z, W);
