 ...
 */
	struct FGenericPlatformMath
	{
		 ...
		 */
		static constexpr inline int32_t TruncToInt(float F)
		{
			return (int32_t)F;
		}
		...
		*/
		...
		static inline uint32_t FloorLog2(uint32_t Value)
		{
			...
			*/
			...
			int _inline method3(uint32_t v)
			{
		...
		static inline uint64_t RoundUpToPowerOfTwo64(uint64_t V)
		{
			return uint64_t(1) << CeilLogTwo64(V);
		}

		/** Spreads bits to every other. */
		static inline uint32_t MortonCode2(uint32_t x)
		{
			x &= 0x0000ffff;
			x = (x ^ (x << 8)) & 0x00ff00ff;
			x = (x ^ (x << 4)) & 0x0f0f0f0f;
		...
		static inline uint32_t ReverseMortonCode2(uint32_t x)
		{
			x &= 0x55555555;
			x = (x ^ (x >> 1)) & 0x33333333;
			x = (x ^ (x >> 2)) & 0x0f0f0f0f;
		...
		static inline uint32_t MortonCode3(uint32_t x)
		{
			x &= 0x000003ff;
			x = (x ^ (x << 16)) & 0xff0000ff;
			x = (x ^ (x << 8)) & 0x0300f00f;
		...
		static inline uint32_t ReverseMortonCode3(uint32_t x)
		{
			x &= 0x09249249;
			x = (x ^ (x >> 2)) & 0x030c30c3;
			x = (x ^ (x >> 4)) & 0x0300f00f;
	...
	template<>
	inline float FGenericPlatformMath::Abs(const float A)
	{
		return fabsf(A);
	}
}
